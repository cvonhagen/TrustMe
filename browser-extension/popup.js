...
async function handleLogin() {
  ...
  try {
    ...
    if (data.two_fa_enabled) {
      showError(
        "2FA wird in der Extension noch nicht unterstützt. Bitte verwenden Sie die Web-App."
      );
      showLoginForm();
      return;
    }
...
function createPasswordItem(password) {
  ...
  const username = password.encrypted_username
    ? "[Verschlüsselt]"
    : "Kein Benutzername";
  ...
  item.innerHTML = `
        ...
        <div class="password-actions">
            <button class="btn-small btn-fill" data-id="${
              password.id
            }">Ausfüllen</button>
            <button class="btn-small btn-copy" data-id="${
              password.id
            }">Kopieren</button>
        </div>
    `;
...
async function copyPassword(password) {
  try {
    ...
    await navigator.clipboard.writeText(
      "[Verschlüsselt - Entschlüsselung erforderlich]"
    );
